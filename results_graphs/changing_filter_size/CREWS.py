...
d = [x for x in d if int(x[5])%5000==0]
...
for i in range(len(entries)):
    ...
    x = [a[0] for a in results]
    y = [a[1] for a in results]
    axs[2].scatter(y,x, label=str(round(get_ep(entries[i][0][0]))),marker=markers[i])

axs[0].set(xlabel='Filter Size', ylabel='TP HSTS \nDetections')
axs[1].set(xlabel='Filter Size', ylabel='Significant FPs')
