...
$$f(S,e,U,D,A) = S(max(u)) \forall_u d< \frac{u}{2}$$


final results:

by applying $f$ to a range of inputs we obtain data showing the best results our protocol can produce for any value of epsilon and the number of sites being considered. In figure x we can see the results from epsilon values 1-9 and sites values 
