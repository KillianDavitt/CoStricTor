...
func main() {
	...
	qs := [9]float64{0.3554609871366468, 0.9820137900379085, 0.858486449758214, 0.9317384593585715, 0.9590150616959465, 0.9732611503389369, 0.9820137900379085, 0.9879368700291202, 0.9922115761726855}
	//for i:=1; i<200; i++ {
	...
	for i:=0; i<len(filterSizes); i++ {
		for j:=0; j<len(ps); j++ {
			p := []interface{}{filterSizes[i],90000000,ps[j],qs[j],0.045,0.07}
