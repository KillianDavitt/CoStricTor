...
func main() {
	...
	for i:=0; i<len(filterSizes); i++ {
		for j:=0; j<len(ps); j++ {
			p := []interface{}{filterSizes[i],1000000,ps[j],qs[0],primaryThresh[0],secondaryThreshs[0]}
